...
struct DRAsyncImage<Content:View>: View {
    @ObservedObject private var loader: DRImageLoader
    private let placeholder:()->Content
        
    init(url:URL?, placeholder:@escaping ()->Content) {
        self.loader = DRImageLoader(url:url)
...
class DRImageLoader: ObservableObject {
